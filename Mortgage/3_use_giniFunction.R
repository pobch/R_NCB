...
# Plot:
ggplot(data = subset(gini.num, feature.name == 'DATE_ACCOUNT_OPENED' & gini.score != 0), aes(x=threshold.date, y=gini.score)) + 
  geom_line()

ggplot(data = subset(gini.num, feature.name == 'Loan_amount1' & gini.score != 0), aes(x=threshold.num, y=gini.score)) + 
  geom_line() +
