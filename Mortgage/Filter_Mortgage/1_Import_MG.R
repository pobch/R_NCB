...
#---------------------- split train/test for Mortgage loan:
subdf_mg = df2 %>% 
  subset(ACCOUNT_TYPE == '06') %>% 
  select(-ACCOUNT_TYPE,
         -Num_PL, -Num_HP, -Num_CC, -Num_OD, -Num_OTH_HP, -Num_CL,
         -Num_COL, -Num_RD, -Num_OL, -Num_A_PL, -Num_A_HP, -Num_A_CC, 
         -Num_A_OD, -Num_A_OTH_HP, -Num_A_CL, -Num_A_COL, -Num_A_RD, -Num_A_OL,
         -nloans, -nactiveloans)
subdf_mg = subdf_mg[-which(subdf_mg$ACCOUNT_STATUS == '42'),] # There is only 1 row

set.seed(123)
spl = sample.split(subdf_mg$Ever30plus_n12MTH, SplitRatio = 0.7)
train_mg = subset(subdf_mg, spl == T)
test_mg = subset(subdf_mg, spl == F)
